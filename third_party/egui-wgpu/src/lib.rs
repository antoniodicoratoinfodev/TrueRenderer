...
pub struct SurfaceConfig {
    /// TrueRenderer patch: creation-time preference, explicit SDR sRGB only.
    /// Unsupported format/color-space pairs fall back to compatible 8-bit.
    /// Changing this field requires recreating the renderer/window.
...
pub struct RenderState {
    /// Actual color-space contract, not inferred from the texture format.
    pub surface_diagnostics: String,
...
impl RenderState {
    ...
    pub async fn create(
        ...
        let capabilities = compatible_surface.map(|s| s.get_capabilities(&adapter));
        let requested = config.surface.preferred_format;
        let target_format = requested
            .filter(|format| {
                matches!(
                    format,
                    wgpu::TextureFormat::Rgb10a2Unorm | wgpu::TextureFormat::Rgba16Float
                ) && capabilities.as_ref().is_some_and(|c| {
                    c.color_spaces(*format)
                        .contains(wgpu::SurfaceColorSpaces::SRGB)
                })
            })
            .map_or_else(|| crate::preferred_framebuffer_format(&surface_formats), Ok)?;
        let surface_diagnostics = format!(
            "requested={requested:?}; effective={target_format:?}; color_space=Srgb; SDR only; physical panel/link bits=not observed; supported={:?}",
            capabilities.as_ref().map(|c| &c.format_capabilities)
        );
