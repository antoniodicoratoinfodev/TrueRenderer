// TrueRenderer local modifications: see README-TrueRenderer.md; upstream licenses retained.
use egui::{UserData, ViewportId};
use epaint::ColorImage;
use std::sync::{Arc, mpsc};
use wgpu::{BindGroupLayout, MultisampleState, StoreOp};

/// TrueRenderer: native codes before illustrative ColorImage8, opt-in diagnostics only.
pub struct NativeCapture {
    pub format: wgpu::TextureFormat,
    pub size: [u32; 2],
    pub bytes: Vec<u8>,
}
static NATIVE_ENABLED: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
static NATIVE_CAPTURE: std::sync::Mutex<Option<NativeCapture>> = std::sync::Mutex::new(None);
pub fn enable_native_capture(enabled: bool) {
    NATIVE_ENABLED.store(enabled, std::sync::atomic::Ordering::Release);
    if !enabled {
        *NATIVE_CAPTURE.lock().unwrap() = None;
    }
}
pub fn take_native_capture() -> Option<NativeCapture> {
    NATIVE_CAPTURE.lock().unwrap().take()
}
fn half_to_u8(bits: u16) -> u8 {
    ...
    let v = if exponent == 0 {
        ...
        (1. + fraction / 1024.) * 2f32.powi(exponent as i32 - 15)
    };
    let v = if bits & 0x8000 != 0 { -v } else { v };
...
impl CaptureState {
    ...
    fn create_texture(
        ...
        let padding = BufferPadding::new(
            surface_texture.width(),
            if surface_texture.format() == wgpu::TextureFormat::Rgba16Float {
                8
            } else {
                4
            },
    ...
    pub fn read_screen_rgba(
        ...
        let to_rgba = match format {
            ...
            wgpu::TextureFormat::Rgb10a2Unorm | wgpu::TextureFormat::Rgba16Float => [0, 1, 2, 3],
            _ => {
                log::error!(
        ...
        buffer_slice.map_async(wgpu::MapMode::Read, move |result| {
            ...
            let retain_native = NATIVE_ENABLED.load(std::sync::atomic::Ordering::Acquire);
            let mut native = Vec::new();
            if retain_native {
                native
                    .reserve(padding.unpadded_bytes_per_row as usize * tex_extent.height as usize);
            }
            for padded_row in mapped_range.chunks(padding.padded_bytes_per_row as usize) {
                ...
                for raw in row.chunks(bpp) {
                    let color: [u8; 4] = if format == wgpu::TextureFormat::Rgba16Float {
                        std::array::from_fn(|i| {
                            half_to_u8(u16::from_le_bytes([raw[i * 2], raw[i * 2 + 1]]))
                        })
                    } else if format == wgpu::TextureFormat::Rgb10a2Unorm {
                        ...
                        .map(|c| (c as f32 * 255. / 1023.).round() as u8)
                    } else {
                        raw.try_into().unwrap()
                    };
            ...
            if retain_native {
                *NATIVE_CAPTURE.lock().unwrap() = Some(NativeCapture {
                    format,
                    size: [tex_extent.width, tex_extent.height],
                    bytes: native,
                });
            }
            drop(mapped_range);
