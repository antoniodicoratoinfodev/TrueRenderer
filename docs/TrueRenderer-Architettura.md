...
| Verifica | 37 test Rust, 6 prove IPC, 19 fixture esterne e 6 controlli aggiuntivi; 24 casi sinusoidali; nuove schermate RAW/JPEG/griglia | Pacchetto finale verificato; estendere target e corpus |
| GPU | Apple M4/Metal: 4.096 campioni, errore massimo 9,54e-7 contro soglia 1e-4 | ICC/LUT, display e recovery non qualificati |
| Pacchetto | 0.1.4 release arm64 con due servizi XPC; firma/hash, Finder e copia autonoma verificati; 0.1.2 conservata | Firma di distribuzione e target Windows in R4 |
...
**Incremento 0.1.4 in corso:** implementate cache lossless e temporanei nella cartella aperta, impostazioni di quota/pulizia e README inglese. Passati 37 test Rust (5 nuovi sulla cache) e la prima verifica nativa: JPEG 12 MP circa 2,05 s a freddo / 0,96 s a caldo, DNG 2,59 s / 0,069 s, ogni livello identico bit per bit. Pacchetto 0.1.4 installato: cache, formati, impostazioni, ricampionamento, XPC, Finder e copia autonoma passati. In completamento pubblicazione prima di ulteriori ottimizzazioni. Il requisito precedente che collocava la cache accanto alle foto nel post-v1 viene anticipato esplicitamente; gli originali e le annotazioni rimangono separati dai derivati eliminabili.
...
| §20 — licenze e nome | Licenza proprietaria scelta dal titolare, repository pubblico, Cargo.lock, 206 package e 319 notice conservati, nome TrueRenderer | **Da qualificare prima di distribuire.** SBOM degli artefatti finali, obblighi delle librerie native effettive, codec/brevetti/canali e clearance del marchio **TrueRenderer**. Il cambio di nome nel progetto non costituisce una verifica commerciale del marchio. |
