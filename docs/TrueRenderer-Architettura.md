...
precisione SDR opzionale e FITS 2D è in [ADR 0010](esportazione-precisione-fits.md).
...
CIRAWFilter e non incorpora LibRaw. Fonti ufficiali e obblighi: [verifica LibRaw](progetto-motori-raw.md#adr-0008).
...
| Caricamento cartella | Background | Preparazione delle miniature della cartella in background oppure prima dell'uso con popup in primo piano; indipendente dal prefetch delle foto adiacenti |
...
Popup con pausa/ripresa, annullamento e passaggio fra primo piano/background per la sessione; la preferenza globale si salva con Applica e salva. Il popup modale termina automaticamente al successo, mentre errori/esclusioni restano consultabili. Annullamento conserva l'avanzamento raggiunto; cambio cartella invalida il lavoro precedente, eventi di vecchie generazioni non incrementano il conteggio. Riletture periodiche identiche e riordinamenti non riavviano la preparazione. Cambi a inventario osservato, motore o qualità globale avviano una nuova preparazione. Scansioni parziali o fallite non certificano la cartella completa.
...
Il filtro fotografico CPU calcola l'intermedio orizzontale per fasce di circa 8 MiB, includendo gli aloni richiesti dai coefficienti. Ordine degli accumuli, campioni fp32 e chiavi dei derivati restano invariati; il riferimento scalare conserva l'intermedio completo per il confronto.

Su macOS le allocazioni Rust di almeno 4 MiB, con allineamento fino a 4096 byte, usano mapping anonimi privati `mmap`/`munmap`: il rilascio non dipende dal recupero differito dei grandi blocchi di malloc. Gli altri layout e le altre piattaforme mantengono l'allocatore di sistema; crescita/riduzione trasferiscono i byte senza condividere mapping con i worker. I codec C e i driver conservano i propri allocatori. Core Image usa un contesto limitato al job per render di almeno 12 milioni di pixel e contesti persistenti per immagini più piccole. Nessun riavvio XPC per ogni immagine: revoca, timeout e supervisione restano quelli del broker. Queste scelte non sono un tetto kernel alla memoria complessiva. API pubbliche: [mmap](https://developer.apple.com/library/archive/documentation/System/Conceptual/ManPages_iPhoneOS/man2/mmap.2.html), [munmap](https://developer.apple.com/library/archive/documentation/System/Conceptual/ManPages_iPhoneOS/man2/munmap.2.html).
...
I decode che richiedono più della metà del budget di lavoro disponibile dopo la baseline vengono serializzati prima del probe; il secondo attende con cancellazione e limite di attesa, senza consumare l'allowance di parsing né ricevere il precedente errore di contesa dopo due secondi. I job piccoli rilasciano il permesso dopo il probe e possono procedere in parallelo. La coda hash/cache rimane indipendente. Sono ammessi al massimo due snapshot complessivi tra lettura/hash, pronti, decoder attivi e decoder in attesa: un lease di slot precede la lettura e resta associato alla sorgente fino al rilascio per completamento o cancellazione. Il solo limite alla coda dei pronti non limita le copie trattenute dai thread. Le eccedenze attendono senza leggere la sorgente, controllando cancellazione e coalescenza ogni 25 ms; non vengono rimosse dai consumatori che il decoder può ancora raggruppare. Restano separati il limite di 64 descrittori e quello in byte pari a un terzo del budget globale. Ciò impedisce a un accumulo di sorgenti future di occupare tutto lo spazio necessario al prossimo decode. L'ordine dei lavori resta quello delle priorità esistenti; un job nativo già attivo non viene preemptato solo per uno scroll.

**Dipendenze visibili prima del dettaglio.** Il viewer rinvia la richiesta di dettaglio finché mancano i derivati visibili fino a 2048 pixel già richiesti da ispettore/filmstrip. In questa fase, oppure quando manca la rappresentazione richiesta dopo un cambio zoom/qualità, rilascia gli antenati non richiesti che singolarmente superano un ottavo del budget globale. Protegge tutte le identità condivise già richieste, anche dall'altro pannello di confronto; conserva le rappresentazioni più piccole e le mostra come raffinamento, senza cambiare qualità, motore, selezione o zoom. I fotogrammi già completati restano disponibili per la riproiezione: possiedono texture di uscita, non l'antenato CPU. I lavori attivi mantengono i lease fino al completamento effettivo. Una volta completate le dipendenze riprende la richiesta originale, incluso il livello nativo a 1:1. Un errore effettivo su una miniatura non blocca indefinitamente il dettaglio delle altre foto. È una politica di residenza/ordinamento: non introduce decode RAW ridotto né garantisce il tetto RSS o la contemporanea disponibilità di due immagini native in confronto a budget insufficiente.
