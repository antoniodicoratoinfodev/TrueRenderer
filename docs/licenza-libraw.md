...
**Baseline pubblicata dell'8 settembre:** TrueRenderer 0.1.5 non includeva né collegava LibRaw; su macOS lo sviluppo RAW passava da CIRAWFilter nei servizi XPC.

**Esperimento locale, aggiornato al 13 settembre e non committato:** LibRaw **0.22.2** sostituisce 0.21.1, dal tag ufficiale, senza modifiche ai sorgenti upstream e con collegamento statico su Windows; scelta CDDL, binding e confini registrati in [ADR 0008](adr/0008-libraw-licenza-e-collegamento.md). Manifest/hash e opzioni compilate sono verificabili con `scripts/verify-libraw.py`; inventario Cargo Windows separato in `reports/dependency-inventory-windows.json`. La configurazione aggiunge gli stessi sorgenti alla futura build Mac con motori selezionabili, ancora da verificare nativamente. Licenze e attribuzioni upstream sono conservate in `third_party/libraw`; il demosaicing Rust proprio non copia i pack GPL. Lo stato locale non costituisce approvazione di una distribuzione commerciale.
