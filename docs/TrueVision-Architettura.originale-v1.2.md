# TrueVision — Analisi architetturale e di progettazione

**Documento di architettura software · v1.2 · 5 settembre 2026**

**Stato: proposta pre-R0; le capability sono criteri di accettazione, non funzioni già disponibili.**
...
completare nelle migrazioni. Le fonti documentano API e specifiche, non dimostrano che TrueVision
...
TrueVision è un browser/visualizzatore di immagini professionale. La sua ragione d'essere non è
...
| # | Errore | Sintomo visibile | Contromisura in TrueVision |
|---|---|---|---|
...
1. **Ogni filtro o compositing implementato da TrueVision opera in luce lineare con alpha
...
2. **I calcoli critici implementati da TrueVision avvengono almeno in fp32.** Precisione,
...
Oltre alla resa corretta, un professionista deve poter verificare cosa sta guardando. TrueVision
...
├───────────────────────────────────────────────────────────────────┤
│ tv-app: stato puro, comandi, journal degli effetti                │
├───────────────────────────────┬───────────────────────────────────┤
│ tv-render: wgpu + WGSL        │ tv-store: index/library/cache     │
├───────────────────────────────┴───────────────────────────────────┤
│ tv-core: immagine, colore, RAW, geometria, scheduler              │
├───────────────────────────────────────────────────────────────────┤
│ tv-platform: file handle, monitor, sandbox, pressure, packaging   │
├───────────────────────────────────────────────────────────────────┤
│ pool tv-worker: LibRaw, JPEG, PNG, TIFF, lcms e parser non fidati │
└───────────────────────────────────────────────────────────────────┘
```
...
flowchart TB
    ...
    subgraph D["Dominio — indipendente da piattaforma"]
        APP[tv-app: stato, comandi, journal]
        CORE[tv-core: immagine, colore, RAW, geometria]
        JOB[Scheduler: priorità, cancellazione, budget]
    end
    subgraph S["Adattatori in uscita"]
        PLATFORM[tv-platform: file, monitor, pressure]
        DEC[tv-worker: pool isolato]
        GPU[tv-render: wgpu]
...
|---|---|---|
| `tv-core` | Tipi immagine/colore, geometria, porte `TileProvider`/decoder, pipeline RAW, riferimento CPU proprio | Rust puro; nessuna UI, SQL, wgpu o handle OS; lcms/LibRaw sono implementazioni esterne delle porte |
| `tv-app` | Store, comandi, scheduler, journal degli effetti, sessione | Dipende da porte, non dagli adattatori |
| `tv-render` | Consuma `TileProvider`; residenza GPU, composizione, shader, verifica GPU | Incapsula wgpu per il motore; l'adattatore UI può dipenderne per condividere device/coda |
| `tv-store` | Indice derivato, libreria utente, cache e migrazioni | Unico package che dipende da SQLite |
| `tv-platform` | File handle, watcher, monitor/ICC, pressure API, sandbox e integrazione desktop | Implementazioni `cfg` piccole e isolate |
| `tv-worker` | Eseguibile senza stato di dominio durevole per sniffing, metadata, ICC e decode | Nessuna UI o database; stato nativo transitorio e autorità di processo seguono §5.3 |
...
sequenceDiagram
    ...
    participant App as tv-app
    participant Cache as tv-store / cache
    participant Job as tv-app / scheduler
    participant W as Worker sandbox
    participant Col as tv-core / colore
    participant R as tv-render
...
piramide TrueVision versionata. Un livello TIFF può essere promosso solo con provenienza e test
...
confine del decoder, non un filtro TrueVision retroattivamente reso lineare.
...
```
  File su disco
      ...
      │      [worker lcms o funzioni canoniche tv-core]
      ├─ 7. Premoltiplicazione alpha in lineare                  [tv-core]
      ├─ 8. Risoluzione via TileProvider: diretto, mip o cache    [tv-render/store]
...
metadata sufficienti alla trasformata TrueVision, oppure RGB lineare in uno spazio di uscita
...
garanzia sul corpus reale di TrueVision. Confronti fra articoli sono validi solo se dataset,
...
Questa parte va adattata ai contratti di TrueVision; infrastrutture analoghe esistono e possono
...
- fp32 è il minimo per matrici concatenate, accumuli di filtri, percorso di riferimento e ogni
  futuro demosaic implementato da TrueVision. La baseline LibRaw conserva invece la propria
...
TrueVision deve quindi permettere di **vedere, scegliere, importare e cambiare** lo spazio colore
...
| Sidecar | File `.xmp` accanto all'immagine; campi standard e namespace TrueVision per identità/profilo | dipende dal campo e dal programma | solo dopo il gate §13.4 e secondo policy di pubblicazione/coordinamento |
...
profilo restano nel namespace versionato TrueVision. L'incorporamento futuro è una funzione per
...
TrueVision indicizza in background i profili presenti sul sistema, riusando l'indice noto senza
...
**Regola adottata:** TrueVision non carica mai le curve e non promette di poter leggere in modo
...
TrueVision usa solo segnali non ambigui: profili di sistema noti per identificatore, metadata di
...
presenta una checklist breve. TrueVision non modifica impostazioni globali e non afferma che
l'ambiente sia conforme solo perché non ha rilevato un filtro.

### 8.8 Intenti di rendering

| Intento | Uso in TrueVision |
|---|---|
...
La promessa di fedeltà va dimostrata, non dichiarata. Questa sezione descrive come TrueVision
...
```
blake3( encode_canonical_v1("tv-cache", content_digest, pipeline_revision,
...
TrueVision non copia gli originali in una libreria gestita: il file system resta la fonte dei byte
...
| Stato TrueVision | Proprietà XMP | Regola |
|---|---|---|
...
| UUID asset, revisione, profilo assegnato | namespace privato versionato | semantica TrueVision, ignorabile da altri programmi; un UUID copiato non prova da solo l'identità |
...
la replica è interoperabile, privata TrueVision o non sincronizzata.
...
- **Storage Access Framework.** L'utente concede l'accesso a un albero di cartelle
  ...
  (`takePersistableUriPermission`), e l'adattatore futuro di `tv-platform` espone gli URI come una sorgente astratta. Il
...
- **Nome “TrueVision”**: è soltanto un nome di lavoro. Prima di dominio, store, logo o annuncio
...
truevision/
├─ crates/
│  ├─ tv-core/                    tipi, coordinate, colore, metadati e policy
│  ├─ tv-app/                     reducer, comandi, effect journal e servizi
│  ├─ tv-render/                  wgpu, residenza tile e confronto con CPU reference
│  ├─ tv-store/                   index, library, cache e migrazioni
│  ├─ tv-platform/                porte OS per Windows/macOS
│  └─ tv-worker/                  protocollo e binario decoder isolato
├─ apps/
...
| 10 | **Nome “TrueVision” non disponibile** | Alto | Media | trattarlo come codename; clearance prima di dominio, store o identità visiva |
...
- [Little CMS 2.18 API](https://www.littlecms.com/LittleCMS2.18%20API.pdf) — trasformate, intenti,
  formati e pipeline disponibili; non prova da sola la precisione di TrueVision
...
`TrueVision-Architettura.v1.1-backup.md`; le seguenti correzioni riguardano la proposta, non
...
Il valore di TrueVision non sta nell'elenco delle funzioni, ma in un contratto verificabile:
