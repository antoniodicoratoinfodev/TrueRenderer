...
TrueRenderer is a native desktop image browser and viewer built around a single goal: showing a photograph with a traceable rendering path. Its working pipeline uses linear light at 32-bit floating point, the viewport draws at the display's real physical resolution, and every render records where its pixels came from. Decoder precision and the remaining colour and display qualification gaps are described below.
...
## Local RAW engine experiment — 12 September 2026

**Current Windows verification, 14 September:** 107 Rust tests passed, debug/release builds, PNG gamma and TIFF colour/alpha checks, RAW preview stage/orientation regressions, and cache timestamp/LRU fixes. [Latest fixes and limits](docs/correzioni-gamma-orientamento.md), [results](reports/gamma-orientation-fixes-windows.json). The owner has authorized committing and pushing this development increment; earlier “no commit” notes describe the preceding sessions. Native Mac/XPC and release qualification remain open.

**Evening review fixes, verified on Windows on 14 September:** corrected the worker test match, settings selection/zoom reset, PNG `cICP` handling and associated TIFF alpha; changed the bitmap cache identity. Recovered 86 ordinary Rust tests and completed six integrations, 21 bitmap requests through LPAC, release build and native UI checks. 577,296 GPU surface pixels meet the one-level sRGB8 tolerance against CPU. [Corrections and remaining limits](docs/correzioni-revisione-serale.md), [results and binary hashes](reports/review-followup-fixes-windows.json). Native Mac/XPC validation remains deferred. One cache write was skipped under lock contention in the native test; complete cache reuse is not established. No commit or release gate closure.

**Pre-main corrections, 13 September:** verified on Windows: LPAC confinement, LibRaw 0.22.2, bitmap colour/classification, parser bounds, worker quotas and cache/source invalidation. Passed 89 Rust tests, debug/release builds, 156 Nikon developments and the four-stage native engine-switching test; originals unchanged. [Results and limits](reports/pre-main-fixes-windows.json), [findings and resolution](docs/revisione-pre-main.md), [Windows confinement](docs/adr/0009-isolamento-worker-windows.md). The new Mac bundle remains unverified; its native tests have been deferred by the owner. Clean Windows installation testing and the colour/performance gates remain open.

This working tree contains an **uncommitted experiment, not for committing or publishing**, extending the existing Windows port. In **Impostazioni → Motore RAW**, choose the engine and press **Applica e salva**. Windows offers historical LibRaw bilinear, LibRaw AHD and experimental TrueRenderer fp32. The macOS build definition offers those plus Apple RAW; the new Mac bundle still requires a native build and XPC verification. Platform defaults are preserved. Changing engines refreshes previews with separate recipe identities in the cache; the save-button regression above currently resets the selected photo and zoom.

The independent Rust demosaic currently accepts Nikon D750 and D40 Bayer NEFs and the project's synthetic DNG fixtures. LibRaw 0.22.2 reads the mosaic and calibration; Rust develops in fp32 without an integer RGB stage or RGB clamp. AHD and historical bilinear pass through a 16-bit integer raster. All three Windows engines passed full development of 30 authorized D750 NEFs and, in the subsequent review, 22 D40 NEFs; the originals remained unchanged. The [D40 review](docs/verifica-motori-d40.md) also fixes changing engines during a pending folder scan. Synthetic tests show improvements on correlated detail but worse results on independent chromatic patterns. **Camera colour accuracy and superiority over AHD or Apple have not been established.** All outputs remain Preview.

Windows development executable: `target/debug/truerenderer.exe`; keep `tr-worker.exe` beside it. RAW processing now requires the LPAC/Job Object worker described in [ADR 0009](docs/adr/0009-isolamento-worker-windows.md), including its measured limits. Details, recipes, reproduction steps and open work: [RAW engine project](docs/progetto-motori-raw.md). Private photographs, crops and detailed reports remain under ignored `var/`. The following Mac format/build evidence describes the published baseline of 9 September, not the new Mac engine configuration.
